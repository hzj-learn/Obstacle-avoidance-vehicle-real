...
public:
    ...
    ros::Publisher pubReplanningGoal;

    nav_msgs::OccupancyGrid occupancyMap2D;

    geometry_msgs::PoseStamped reGoal;
    bool Goal_recevied;
    ...
    PathPlanning()
    {
        subGlobalPath = nh.subscribe<nav_msgs::Path>("global_path", 5, &PathPlanning::pathHandler, this);
        subObstacleMap = nh.subscribe<nav_msgs::OccupancyGrid>("planning/obstacle/map_inflated", 5, &PathPlanning::mapHandler, this);
	subGoal = nh.subscribe<geometry_msgs::PoseStamped>("/goal", 5, &PathPlanning::goalHandler, this);
	...
	pubReplanningGoal = nh.advertise<geometry_msgs::PoseStamped>("/move_base_simple/goal",1);
...
void goalHandler(const geometry_msgs::PoseStamped::ConstPtr& goalMsg)
   {
	reGoal = *goalMsg;
	Goal_recevied = true;
    }
	...
	if(Goal_recevied)
          pubReplanningGoal.publish(reGoal);
