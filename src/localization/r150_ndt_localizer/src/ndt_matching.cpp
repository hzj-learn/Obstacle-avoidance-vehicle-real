...
static ros::Publisher ndt_vel_pub;
...
static std::string _imu_topic = "/imu";
static std::string _points_topic = "filtered_points";
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  if (map_loaded == 1 && init_pos_set == 1)
  {
    ...
    */
    ...
    if (_use_local_transform == true)
    {
      br.sendTransform(tf::StampedTransform(local_transform * transform, current_scan_time, "/map", "/base_footlink"));
    }
    else
    {
      br.sendTransform(tf::StampedTransform(transform, current_scan_time, "/map", "/base_link"));
    ...
    estimate_twist_msg.header.frame_id = input->header.frame_id;
    ...
    ndt_vel_pub.publish(estimate_twist_msg);
    ...
    if(_output_log_data)
    {
      ...
      else
      {
        ofs << input->header.seq << "," << scan_points_num << "," << step_size << "," << trans_eps << "," << std::fixed
            ...
            << odom.pose.pose.position.x << "," << odom.pose.pose.position.y << "," << odom.twist.twist.linear.x << "," << odom.twist.twist.angular.z << ","
...
int main(int argc, char** argv)
{
  ...
  if(_output_log_data)
  {
    ...
    std::string directory_name = "log/ndt_matching";
  ...
  private_nh.getParam("init_pos_set", init_pos_set);

  private_nh.getParam("localizer", _localizer);
  ...
  ndt_vel_pub = nh.advertise<geometry_msgs::TwistStamped>("/current_velocity", 10);
  ...
  ros::Subscriber odom_sub = nh.subscribe("odom", _queue_size * 10, odom_callback);
